import os # Импортируем модуль для работы с путями
from PIL import Image # Импортируем библиотеку для открытия картинок

# Создаем функцию торговца, которая принимает деньги, урон, инвентарь и базу данных
def start_trade(money, damage, inventar, info, BASE_DIR):
    print("\n--- Лавка торговца ---") # Заголовок для красоты
    print("Доступные товары:", list(info.keys())) # Показываем названия товаров из словаря
    
    torg = input("Что хочешь купить? (Enter - выйти): ") # Спрашиваем выбор игрока

    if torg == "": # Если игрок ничего не ввел
        return money, damage # Просто возвращаем деньги и урон назад без изменений

        if torg in info: # Если товар есть в базе данных
            price = info[torg]["Цена"] # Узнаем цену
            
            # Если товар НЕ "Зелье" И он УЖЕ есть в инвентаре
            if torg != "Зелье" and torg in inventar:
                print(f"У вас уже есть {torg}! Зачем вам второй?") # Отказываем в покупке
                return money, damage # Возвращаемся в меню без изменений
            
            if money >= price: # Если денег хватает
                money -= price # Списываем золото
                damage += info[torg].get("Урон", 0) # Добавляем урон сразу (как ты и хотел)
                inventar.append(torg) # Добавляем предмет в список сумки
                print(f"Успешно! Куплен {torg}. Ваш общий урон: {damage}") # Отчет
            
            try: # Пробуем показать картинку
                put = os.path.join(BASE_DIR, info[torg]["картинка"]) # Собираем путь к файлу
                img = Image.open(put) # Открываем картинку
                img.show() # Показываем её на экране
            except: # Если картинка не найдена или сломана
                print("Картинку не удалось загрузить.") # Выводим ошибку, но не вылетаем
        else: # Если денег меньше, чем цена
            print("Недостаточно золота!") # Сообщаем о бедности
    else: # Если товара с таким названием нет в словаре
        print("Такого товара нет в ассортименте.") # Ошибка ввода
    
    return money, damage # ВАЖНО: отдаем измененные деньги и урон обратно в главное меню
